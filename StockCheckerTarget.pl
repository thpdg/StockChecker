...
# print "\n\nStocker Checker Parsing Script For Target\n";

while ($data =~ m/<title(.+?)<\/title>/g) {
    print "\nFor $1 ";
}

if ($data =~ m/alt="data-test="shipItButton">Ship it<\/button>/g) {
    print color('bold green');
    system('say BTarget');
    print "\r\n\t\tPossibly Available on Target\n";
} else {
    print color('bold red');
    print "\r\n\t\tCurrently Sold Out on Target\n";
}
